...
const Mybooking = () => {
  ...
  return !isLoading ? (
    <div className='relative px-6 md:px-16 lg:px-40 pt-30 md:pt-40 min-h-[80vh]'>
      ...
      <div>
        <BlueC bottom='0px' left='600px' />
      </div>
      <h1 className='text-lg font-semibold mb-4'>Đặt Chổ Của Tôi</h1>
        ...
        <div key={index} className='flex flex-col md:flex-row justify-between bg-primary/8 border border-primary/20
          ...
          <div className='flex flex-col md:items-end md:text-right justify-between p-4'>
            <div className='flex items-center gap-4'>
              <p className='text-2xl mb-3'>{currency}{item.amount}</p>
              {!item.isPaid && <button className='bg-primary px-4 py-1.5 mb-3 text-sm rounded font-medium cursor-pointer'>Thanh Toán
              </button>}
            </div>
